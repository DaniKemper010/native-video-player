...
class NativeVideoPlayerController {
  ...
  void _updateState(NativeVideoPlayerState newState) {
    ...
    if (oldState.duration != newState.duration) {
      ...
      if (oldState.duration == Duration.zero && newState.duration != Duration.zero) {
        debugPrint('🎯 Duration became available! Notifying all ${_controlEventHandlers.length} control listeners');
  ...
  void addActivityListener(void Function(PlayerActivityEvent) listener) {
    if (!_activityEventHandlers.contains(listener)) {
      ...
      debugPrint('👂 Added activity listener. Total listeners: ${_activityEventHandlers.length}');
      ...
      if (!_isDisposed && _state.duration != Duration.zero) {
        debugPrint('🔔 Emitting current activity state to new listener: ${_state.activityState}');
        final currentActivityEvent = PlayerActivityEvent(state: _state.activityState, data: null);
        listener(currentActivityEvent);
      } else {
        debugPrint('🔕 NOT emitting activity to new listener: _isDisposed=$_isDisposed, duration=${_state.duration}');
      }
    } else {
  ...
  void addControlListener(void Function(PlayerControlEvent) listener) {
    if (!_controlEventHandlers.contains(listener)) {
      ...
      debugPrint('👂 Added control listener. Total listeners: ${_controlEventHandlers.length}');
